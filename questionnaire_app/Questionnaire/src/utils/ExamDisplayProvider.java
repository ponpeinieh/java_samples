...
public class ExamDisplayProvider implements DisplayProvider {
    ...
    public void printQuestList(Questionnaire qn, QuestionnaireResult result) {
        List<AnswerRecord> records = result.getAnswerRecords();
        int order = 1;
        for (var q : qn.getQuestions()) {
            printQuestSimple(q, records.get(order - 1).getAnswer(), order);
            order++;
        }
    }
    ...
    public void printUnansweredQuestList(Questionnaire qn, QuestionnaireResult result) {
        ...
        int order = 1;
        for (var q : qn.getQuestions()) {
            Answer answer = records.get(order - 1).getAnswer();
    ...
    public void printCurrentAnswer(Answer answer) {
        ...
        if (answer.isUserAnswered()) {
            switch (answer.getQuestionType()) {
                case SINGLE: {
                    List<Choice> choiceAnswers = answer.getChoiceAnswer();
                    List<Choice> questionChoices = answer.getQuestion().getChoices();
