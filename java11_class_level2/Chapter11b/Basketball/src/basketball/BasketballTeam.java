...
public class BasketballTeam implements Team{
    ...
    public int compareTo(Team theTeam){
        int returnValue = -1;
        if (this.getPointsTotal()< ((BasketballTeam)theTeam).getPointsTotal()) {
            returnValue = 1;
    ...
    public BasketballTeam(String teamName, Player[] players) {
        this(teamName);
    ...
    GameEvent[] possEvents = currEvent.getNextEvents();
                ...
                // Easy way to modify code to favor a particular team
			...
			for (GameEvent thisEvent: possEvents ) {
				if ( thisEvent instanceof Shoot) { // Is Shoot valid here?
					if (ballPos < 60 ) {			// Are we a long way from goal? If so remove Shoot
						...
						possEvents = new BasketballEvent[eventList.size()];
